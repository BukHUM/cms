...
@section('title', 'Edit Performance Setting - แก้ไขการตั้งค่าประสิทธิภาพ')
...
<div class="main-content">
    <main class="main-content-area">
        ...
        <div class="mb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="mt-4 sm:mt-0 flex space-x-2">
                    <a href="{{ route('backend.settings-performance.show', $performance) }}" class="btn-secondary">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form action="{{ route('backend.settings-performance.update', $performance) }}" method="POST" id="performance-form">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $performance->name) }}" 
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="key" id="key" value="{{ old('key', $performance->key) }}" 
                        ...
                        <div class="form-group">
                            ...
                            <select name="type" id="type" class="form-input @error('type') border-red-500 @enderror" required>
                                ...
                                @foreach($types as $key => $label)
                                    <option value="{{ $key }}" {{ old('type', $performance->type) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <select name="category" id="category" class="form-input @error('category') border-red-500 @enderror" required>
                                ...
                                @foreach($categories as $key => $label)
                                    <option value="{{ $key }}" {{ old('category', $performance->category) == $key ? 'selected' : '' }}>
                                        {{ $label }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group md:col-span-2">
                            ...
                            <div id="value-input-container">
                                <input type="text" name="value" id="value" value="{{ old('value', $performance->value) }}" 
                        ...
                        <div class="form-group md:col-span-2">
                            ...
                            <div id="default-value-input-container">
                                <input type="text" name="default_value" id="default_value" value="{{ old('default_value', $performance->default_value) }}" 
                        ...
                        <div class="form-group md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="3" 
                                      ...
                                      placeholder="คำอธิบายการตั้งค่านี้">{{ old('description', $performance->description) }}</textarea>
                            @error('description')
                                <p class="form-error">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $performance->sort_order) }}" 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div class="form-group">
                                ...
                                <input type="checkbox" name="validation_rules[required]" value="1" 
                                       {{ old('validation_rules.required', $performance->validation_rules['required'] ?? false) ? 'checked' : '' }}
                                       class="form-checkbox h-4 w-4 text-blue-600">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="validation_min">ค่าต่ำสุด</label>
                                <input type="number" name="validation_rules[min]" value="{{ old('validation_rules.min', $performance->validation_rules['min'] ?? '') }}" 
                                       class="form-input" placeholder="ค่าต่ำสุด">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="validation_max">ค่าสูงสุด</label>
                                <input type="number" name="validation_rules[max]" value="{{ old('validation_rules.max', $performance->validation_rules['max'] ?? '') }}" 
                                       class="form-input" placeholder="ค่าสูงสุด">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="validation_min_length">ความยาวต่ำสุด</label>
                                <input type="number" name="validation_rules[min_length]" value="{{ old('validation_rules.min_length', $performance->validation_rules['min_length'] ?? '') }}" 
                                       class="form-input" placeholder="ความยาวต่ำสุด">
                            </div>
                            <div class="form-group">
                                <label class="form-label" for="validation_max_length">ความยาวสูงสุด</label>
                                <input type="number" name="validation_rules[max_length]" value="{{ old('validation_rules.max_length', $performance->validation_rules['max_length'] ?? '') }}" 
                    ...
                    <div class="mt-6">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="options" id="options" rows="4" 
                                      ...
                                      placeholder='{"option1": "ค่า 1", "option2": "ค่า 2"}'>{{ old('options', $performance->options ? json_encode($performance->options, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) : '') }}</textarea>
                            @error('options')
                                <p class="form-error">{{ $message }}</p>
                            @enderror
